...
Explanation:
1. We iterate from 0 to the length of the list increasing =i= every iteration.
2. Each iteration we add =i= to the value on the list at index =i=.
3. Once we are done, we return the list. Might be a good idea to save the length
   of the list in a local variable instead of using it directly as an exit
   condition (I am not sure if =length=) gets called every iteration.
